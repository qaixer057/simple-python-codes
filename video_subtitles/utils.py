...
def subtitle_generation(transcript_text, bin_size=3):
    ...
    for result in transcript_text:
        try:
            if result.alternatives[0].words[0].start_time.seconds:
                # bin start -> for first word of result
                start_sec = result.alternatives[0].words[0].start_time.seconds
                start_microsec = (
                    result.alternatives[0].words[0].start_time.nanos * 0.001
                )
            else:
            ...
            last_word_end_microsec = (
                result.alternatives[0].words[-1].end_time.nanos * 0.001
            )
            ...
            for i in range(len(result.alternatives[0].words) - 1):
                try:
                    ...
                    word_start_microsec = (
                        result.alternatives[0].words[i + 1].start_time.nanos * 0.001
                    )  # 0.001 to convert nana -> micro
                    word_end_sec = result.alternatives[0].words[i + 1].end_time.seconds
                    word_end_microsec = (
                        result.alternatives[0].words[i + 1].end_time.nanos * 0.001
                    )
                    ...
                    if word_end_sec < end_sec:
                        ...
                        previous_word_end_microsec = (
                            result.alternatives[0].words[i].end_time.nanos * 0.001
                        )
